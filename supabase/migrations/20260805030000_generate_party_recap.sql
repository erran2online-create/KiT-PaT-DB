-- P7: generate_party_recap — aggregate winners / attendance / top badge / photo_ids
-- into memory_artifacts (artifact_type=party_recap). Pure data; no image rendering.
-- Existing memory_artifacts.payload stores the recap JSON (no separate recap_json column needed).
...
begin
  ...
  -- Cached read for non-hosts; only host may regenerate
  if existing.id is not null and not is_host then
  ...
  if existing.id is not null and is_host then
...
COMMENT ON FUNCTION public.generate_party_recap(uuid) IS
  'Assemble party recap JSON (winners, attendance, top badge, photo_ids) into memory_artifacts.party_recap. Members read cached; host regenerates.';

REVOKE ALL ON FUNCTION public.generate_party_recap(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.generate_party_recap(uuid) TO authenticated, service_role;
